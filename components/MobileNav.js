...
export default function MobileNav({ links = [], locale = 'zh' }) {
  ...
  // 抽屜內容（透過 Portal 渲染到 body，跳出 sticky header 的 stacking context）
  const drawer = open && mounted ? createPortal(
    ...
    >
      ...
      >
        {/* 頂部標題 */}
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#ffffff' }}>
          <div>
        ...
        {/* 導覽連結 */}
        <nav style={{ flex: 1, overflowY: 'auto', padding: '12px 8px', backgroundColor: '#ffffff' }}>
          {links.map(l => {
